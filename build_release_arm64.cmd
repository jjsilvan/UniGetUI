...
dotnet publish src/UniGetUI/UniGetUI.csproj /noLogo /property:Configuration=Release /property:Platform=arm64 -v m
if %errorlevel% neq 0 (
...
mkdir unigetui_bin
robocopy src\UniGetUI\bin\arm64\Release\net8.0-windows10.0.26100.0\win-arm64\publish unigetui_bin *.* /MOVE /E
...
mkdir output
cd unigetui_bin
7z a -tzip "..\output\UniGetUI.arm64.zip" "*"
cd ..
if %errorlevel% neq 0 (
    echo "Compression of unigetui_bin into output/UniGetUI.arm64.zip has failed!"
    pause
)
...
if exist %INSTALLATOR% (
    ...
    pwsh.exe -Command echo """UniGetUI.arm64.zip SHA256: ``$((Get-FileHash 'output\UniGetUI.arm64.zip').Hash)``"""
    echo .
    pause
